...
int new_msg ( uint8_t *buff, size_t len )
{
    ...
    buff++;
    // Remove termination characters if present after skipping ':'
    if (len < 3) { return -6; } // too short
    if ((buff[len - 3] == '\r') && (buff[len - 2] == '\n')) {
        len -= 3;  // ':' removed via pointer advance; now drop CRLF from length
    } else {
        len -= 1;  // only ':' removed; leave length consistent with new pointer
    }
...
static const double K_VALS[] = { 0.0076000000, 0.0100000000, 0.0458868358, 0.0619309863,
...
static inline double Tc_for_level(int L){
    ...
    const double b = 0.3200000000;   // N·m base
...
bike_data_t getBikeData()
{
    ...
    data.watts = calc_watts_physics_params((double)act_rpm, (int)disp_res);
    return data;
}